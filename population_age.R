...
df_ts009_ltla_syoa_pyramid_data <- df_ts009_ltla_syoa %>% 
  ...
  bind_rows(
    df_ts009_ltla_syoa %>% 
# Don't filter by local areas just yet as we need the national position to calculate the ranks for
# each local authority
#      semi_join(df_swahsn_lu, by = c('LAD22CD' = 'LAD22CD', 'LAD22NM' = 'LAD22NM')) %>%
      group_by(LAD22CD, LAD22NM, AGE, GENDER) %>%
      summarise(POPN = sum(POPN), .groups = 'keep') %>%
      ungroup() %>%
      transmute(LEVEL = 2, AREA_CODE = LAD22CD, AREA_NAME = LAD22NM, AGE , GENDER, POPN)
  )
...
df_ts009_ltla_syoa_summary <- df_ts009_ltla_syoa_pyramid_data %>% 
  ...
  group_by(LEVEL) %>%
  mutate(
    RANK_MED = rank(desc(MED), ties.method = 'first'),
    RANK_MED_LABEL = sprintf('(%d of %d)', RANK_MED, n()),
    RANK_IQR = rank(desc(IQR), ties.method = 'first'),
    RANK_IQR_LABEL = sprintf('(%d of %d)', RANK_IQR, n())
  ) %>%
  ungroup()

df_ts009_ltla_syoa_summary <- df_ts009_ltla_syoa_summary %>% 
...
plt_ts009_boxplot <- ggplot(
  ...
  labs(title = str_wrap('Boxplot of 5th Centile, 1st Quartile, Median, 3rd Quartile, and 95th Centile Age of Population for England, Regions, and Local Authority Districts (LAD)', 80),
